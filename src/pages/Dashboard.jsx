...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Bem-vindo ao MedCore</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-6">
          <h3 className="text-lg font-bold text-slate-800 mb-4">Próximas Consultas</h3>
          <div className="space-y-4">
            {appointments.slice(0, 5).map(app => {
              const patient = patients.find(p => p.id === app.patientId);
              const doctor = doctors.find(d => d.id === app.doctorId);
              return (
                <div key={app.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{doctor?.name} • {app.time}</p>
                    </div>
                  </div>
        ...
        <div className="card p-6">
          ...
          <div className="relative pl-8 space-y-6 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
            <div className="relative">
              <div className="absolute -left-[25px] top-1 w-3 h-3 rounded-full bg-primary ring-4 ring-white"></div>
              <p className="text-sm font-bold text-slate-700">Novo paciente cadastrado</p>
              <p className="text-xs text-slate-500">Há 10 minutos</p>
            </div>
            <div className="relative">
              <div className="absolute -left-[25px] top-1 w-3 h-3 rounded-full bg-success ring-4 ring-white"></div>
              <p className="text-sm font-bold text-slate-700">Consulta realizada com sucesso</p>
              <p className="text-xs text-slate-500">Há 1 hora</p>
            </div>
            <div className="relative">
              <div className="absolute -left-[25px] top-1 w-3 h-3 rounded-full bg-secondary ring-4 ring-white"></div>
              <p className="text-sm font-bold text-slate-700">Novo médico adicionado ao sistema</p>
              <p className="text-xs text-slate-500">Ontem às 16:45</p>
            </div>
          </div>
        </div>
      </div>
