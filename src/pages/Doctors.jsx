    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Gerencie o corpo clínico da MedCore.</p>
        </div>
        <button 
              ...
              {filteredDoctors.length > 0 ? (
                filteredDoctors.map(doctor => (
                  <tr key={doctor.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-500">{doctor.specialty}</td>
                    <td className="px-6 py-4 text-slate-500">{doctor.crm}</td>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">CRM</label>
              <input 
                required
                className="input-field" 
                value={formData.crm} 
